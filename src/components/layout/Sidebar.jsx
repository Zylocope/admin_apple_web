...
const NAV_ITEMS = [
  { to: '/',          icon: IconLayoutDashboard, key: 'dashboard',  managerOnly: false },
  { to: '/products',  icon: IconPackage,         key: 'products',   managerOnly: false },
  { to: '/orders',    icon: IconShoppingCart,    key: 'orders',     managerOnly: false },
  { to: '/tickets',   icon: IconTicket,          key: 'playground', managerOnly: false },
  { to: '/customers', icon: IconUsers,           key: 'customers',  managerOnly: false },
  { to: '/reports',   icon: IconChartBar,        key: 'reports',    managerOnly: true  },
  { to: '/staff',     icon: IconUserCog,         key: 'staff',      managerOnly: true  },
];
...
export default function Sidebar({ collapsed, onToggle }) {
  ...
  const visibleItems = NAV_ITEMS.filter(i => !i.managerOnly || isManager);
      ...
      <nav className="flex-1 py-3 px-2 overflow-y-auto">
        {visibleItems.map(({ to, icon: Icon, key }) => (
          <NavLink
            ...
            className={({ isActive }) =>
              `relative flex items-center gap-3 px-4 py-2.5 rounded-lg mb-1 text-[15px] transition-colors active:scale-[0.98] ${
                isActive
