...
import { IconPlus, IconPencil, IconCircleX, IconRefresh, IconCalendarEvent } from '@tabler/icons-react';
...
export default function Tickets() {
  ...
  const refundTicket = (id) => setTickets(prev => prev.map(t => t.id === id ? { ...t, status: 'Refunded' } : t));
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}
          ...
          {['Confirmed', 'Used', 'Cancelled', 'Refunded'].map(s => <option key={s} value={s}>{tr(`badge.${s}`)}</option>)}
        </select>
                  ...
                  {isManager && (
                    <td className="px-4 py-3.5">
                      <div className="flex items-center gap-1.5">
                        ...
                        {t.status === 'Used' && (
                          <button onClick={() => setConfirmRefund(t)} className="p-1.5 rounded-lg text-mute hover:text-brand hover:bg-brand-light transition-colors" title={tr('orders.refund')}>
      ...
      <ConfirmDialog open={!!confirmRefund} onClose={() => setConfirmRefund(null)}
        onConfirm={() => refundTicket(confirmRefund.id)}
        title={tr('tickets.refundTitle')} message={tr('tickets.refundMsg', { id: confirmRefund?.id, amount: confirmRefund ? formatMMK(confirmRefund.amount) : '' })}
        confirmLabel={tr('tickets.issueRefund')} />
    </div>
  );
}
